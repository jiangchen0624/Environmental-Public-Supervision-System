  ...
  <div class="dv-page">
    ...
    <div class="main-container">
      ...
      <el-row :gutter="12" class="row">
        <el-col :span="4"><div class="card c1"><div class="card-n">{{ detectStats.total }}</div><div class="card-t">检测累计</div><div class="card-sub">上报记录总数</div></div></el-col>
        <el-col :span="4"><div class="card c2"><div class="card-n">{{ detectStats.good }}<small> {{ goodRate }}%</small></div><div class="card-t">结果良好</div><div class="card-sub">AQI等级 ≤ 2</div></div></el-col>
        <el-col :span="4"><div class="card c3"><div class="card-n">{{ detectStats.over }}<small> {{ overRate }}%</small></div><div class="card-t">结果超标</div><div class="card-sub">AQI等级 ≥ 3</div></div></el-col>
        <el-col :span="4"><div class="card c4"><div class="card-n">{{ coverStats.coveredProvinces }}<small>/{{ coverStats.totalProvinces }}</small></div><div class="card-t">省覆盖率</div><div class="card-sub">{{ coverStats.provinceRate }}%</div></div></el-col>
        <el-col :span="4"><div class="card c5"><div class="card-n">{{ coverStats.coveredCities }}<small>/{{ coverStats.totalCities }}</small></div><div class="card-t">城市覆盖率</div><div class="card-sub">{{ coverStats.cityRate }}%</div></div></el-col>
      ...
      <el-row :gutter="12" class="row">
        <el-col :span="14">
          <el-card shadow="hover"><template #header><h3>📈 月度检测数量趋势</h3></template><div ref="trendChart" class="chart" /></el-card>
        </el-col>
        <el-col :span="10">
        ...
        <el-col :span="24">
          <el-card shadow="hover">
            ...
            <div class="map-legend">
              ...
              <span class="legend-item"><span class="legend-color" style="background:#1b5e20"></span>0</span>
              <span class="legend-item"><span class="legend-color" style="background:#4caf50"></span>1-2</span>
              <span class="legend-item"><span class="legend-color" style="background:#ff9800"></span>3-5</span>
              <span class="legend-item"><span class="legend-color" style="background:#f44336"></span>6-10</span>
              <span class="legend-item"><span class="legend-color" style="background:#b71c1c"></span>&gt;10</span>
              <span class="legend-item"><span class="legend-color" style="background:#37474f"></span>无数据</span>
            </div>
          </el-card>
        </el-col>
      </el-row>
...
interface GeoJSONData { type: 'FeatureCollection'; features: GeoFeature[] }
...
function normCity(n: string) { return n.replace(/市$/, '').replace(/盟$/, '').replace(/地区$/, '') }
...
const selCityGrid = computed<CityGridItem[]>(() => {
  ...
  return cities.map((c: string) => ({
    name: c, covered: coveredCityNorm.value.has(normCity(c)),
  }))
})
...
function renderDistChart(data: Record<string, number>) {
  ...
  c2.setOption({
    ...
    graphic: total > 0 ? [{
      ...
      style: { text: `共${total}条`, textAlign: 'center', fill: '#90caf9', fontSize: 13, fontWeight: 'bold' },
    }, {
...
function renderTrendChart(data: Record<string, number>) {
  ...
  c3.setOption({
    ...
    yAxis: { type: 'value', name: '检测数(条)', nameTextStyle: { color: '#78909c', fontSize: 11 }, min: 0, max: Math.max(...vals, 10) + 2 },
    series: [{
      data: Object.values(data), type: 'line', smooth: true, areaStyle: { opacity: 0.35 },
      itemStyle: { color: '#f44336' }, lineStyle: { width: 3 },
      markLine: vals.length > 0 ? { silent: true, data: [{ type: 'average', name: '均值', lineStyle: { color: '#ff9800', type: 'dashed' } }] } : undefined,
    }],
  })
}
...
function renderDetectPieChart() {
  ...
  c4.setOption({
    graphic: [{
      ...
      style: { text: `${goodPct}%`, textAlign: 'center', fill: '#fff', fontSize: 13, fontWeight: 'bold' },
    }, {
...
async function renderMap() {
  ...
  try {
    ...
    if (drillProvince.value) {
      ...
      echarts.registerMap('drill-province', drillGeo as unknown as Parameters<typeof echarts.registerMap>[1])
      ...
      instance.setOption({
        ...
        series: [{
          ...
          emphasis: { label: { show: true, color: '#fff', fontSize: 9 }, itemStyle: { areaColor: '#ff9800' } }
        }]
      })
...
/* 时间范围选择条 */
.time-bar{display:flex;align-items:center;justify-content:center;padding:10px 0 2px;gap:8px}
.time-label{color:#90caf9;font-size:13px}
.time-range-text{color:#78909c;font-size:11px;margin-left:8px}

.card{border-radius:10px;padding:14px 16px;color:#fff;text-align:center;display:flex;flex-direction:column;justify-content:center;min-height:90px}
...
.card-n{font-size:30px;font-weight:700;line-height:1.1}.card-n small{font-size:14px;font-weight:400;opacity:0.8;margin-left:2px}
.card-n-s{font-size:28px;font-weight:700}.card-t{font-size:12px;opacity:0.9;margin-top:2px}
.card-sub{font-size:10px;opacity:0.65;margin-top:1px}
...
.province-grid-sm{display:flex;flex-wrap:wrap;gap:4px;margin-top:10px;justify-content:center}
.pg-sm{width:44px;height:24px;display:flex;align-items:center;justify-content:center;font-size:10px;border-radius:3px;background:#37474f;color:#90a4ae;cursor:pointer;transition:.2s;position:relative}
.pg-sm:hover{transform:scale(1.1);z-index:1}
.pg-sm.covered{background:#4caf50;color:#fff;font-weight:600}
.pg-sm.active{outline:2px solid #ff9800;outline-offset:1px}
.pg-cnt{font-size:8px;opacity:0.8;margin-left:2px}
.grid-legend-sm{display:flex;gap:14px;justify-content:center;margin-bottom:8px;font-size:10px;color:#90a4ae;align-items:center}
.dot-sm{width:10px;height:10px;border-radius:2px;background:#37474f;display:inline-block}.dot-sm.covered{background:#4caf50}
...
.legend-color{width:14px;height:10px;border-radius:2px;display:inline-block}
</style>
